...
function setFreeAccessEnabled(v) { localStorage.setItem(FREE_KEY, v ? "1" : "0"); }
function getAuth() {
...
function initLogin() {
  ...
  // Acceso libre (activable desde Ajustes)
  const btnFree = $("btnFreeAccess");
  const refreshFreeBtn = () => { btnFree.style.display = isFreeAccessEnabled() ? "inline-flex" : "none"; };
  refreshFreeBtn();
...
// ========================== RESET DÍA (bloc diario) ==========================
async function resetDay() {
  const tech = getTech();
  const date = todayStr();

  if (!confirm(`♻️ ¿Resetear TODO lo de HOY (${date})?\n\nSe borrarán: Recirculación, Medidas y Purga (solo hoy).`)) return;

  // parar alarmas
  try { stopAlarm(); } catch {}
  try { $("alarmOverlay")?.classList.remove("show"); } catch {}
  ...
  try {
    ...
    purgaTimer.running = false;
    purgaTimer.paused = false;
    purgaTimer.code = "";
    purgaTimer.elapsedMs = 0;
    purgaTimer.durationMs = 0;
    purgaTimer.minimized = false;

    $("purgaTimerCode") && ($("purgaTimerCode").textContent = "—");
    $("purgaTimerDisplay") && ($("purgaTimerDisplay").textContent = "00:00");
    $("purgaTimerStatus") && ( $("purgaTimerStatus").textContent = "Listo", $("purgaTimerStatus").className = "badge badge-gray");
    $("purgaProgress") && ($("purgaProgress").style.width = "0%");
    $("miniPurga")?.classList.remove("show");
  } catch {}

  // limpiar inputs visibles
  try { $("puntoCode").value = ""; } catch {}
  try { $("recirNota").value = ""; } catch {}
  try { $("sealResult")?.classList.add("hidden"); } catch {}

  // medidas
  for (const id of ["medidaCodigo","mPH","mTemp","mCloroL","mCloroT","mTurb","mCond","mNota"]) {
    try { const el=$(id); if (el) el.value = ""; } catch {}
  }

  // purga
  for (const id of ["purgaCodigo","purgaNota"]) {
    try { const el=$(id); if (el) el.value = ""; } catch {}
  }

  // borrar BD del día
  const res = await dbClearToday(tech, date);

  // refrescar listas
  try { await refreshRecirList(); } catch {}
  try { await refreshMedidasList(); } catch {}
  try { await refreshPurgaList(); } catch {}

  // cerrar ajustes si está abierto
  try { $("modalSettings")?.classList.remove("open"); } catch {}

  soundWarn();
  toast(`Día reseteado ✅ (Recir:${res.recirculacion} · Medidas:${res.medidas} · Purga:${res.purga})`, "ok");
}
...
$("btnQRManual")?.addEventListener("click", () => {
  ...
  if (scanTarget === "recir") $("puntoCode").value = c;
    ...
    else if (scanTarget === "medidas") $("medidaCodigo").value = c;
    else if (scanTarget === "incidencias") $("incCodigo").value = c;
...
async function resetAllDataUI() {
  if (!confirm("♻️ RESET TOTAL\n\nBorra TODOS los datos (histórico incluido):\n- Recirculación\n- Medidas\n- Purga\n- Análisis mensual\n- Incidencias\n\n✅ Mantiene sesión y ajustes.\n\n¿Continuar?")) return;

  const txt = prompt("Escribe RESET para confirmar (en mayúsculas):", "");
  if (txt !== "RESET") {
    soundWarn();
    toast("Cancelado. No se ha borrado nada.", "info");
    return;
  }

  // Parar alarmas/timers si están activos
  try { stopAlarm(); $("alarmOverlay")?.classList.remove("show"); } catch {}
  try { cancelAnimationFrame(recirTimer?.raf); } catch {}

  // Limpiar UI rápida (sin tocar ajustes)
  try {
    $("timerCode").textContent = "—";
    $("timerDisplay").textContent = "00:00";
    $("timerStatus").textContent = "Listo";
  ...
  // BORRAR TODO EN INDEXEDDB
  await dbClearAllData();

  // Refrescar pantallas/listas
  await refreshRecirList();
  await refreshAnalisisList();
  await refreshMedidasList();
  await refreshPurgaList();
  if (typeof refreshIncList === "function") await refreshIncList();

  // Cerrar ajustes si está abierto
  $("modalSettings")?.classList.remove("open");

  soundWarn();
  toast("Reset total ✅ (datos borrados, sesión y ajustes conservados)", "ok");
}

$("btnResetAll")?.addEventListener("click", resetAllDataUI);
$("btnResetAll2")?.addEventListener("click", resetAllDataUI);
