...
function openDB() {
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = () => {
      ...
      // Incidencias (fotos + nota)
      if (!db.objectStoreNames.contains("incidencias")) {
        const s = db.createObjectStore("incidencias", { keyPath: "id", autoIncrement: true });
        s.createIndex("byTechDate", ["tech", "date"], { unique: false });
      }
    };
...
// --- Reset diario (borra solo HOY) ---
export async function dbClearToday(tech, date) {
  const db = await openDB();

  async function delByIndex(storeName, indexName, key) {
    return new Promise((res, rej) => {
      const tx0 = db.transaction(storeName, 'readonly');
      const store0 = tx0.objectStore(storeName);
      const idx = store0.index(indexName);
  ...
  const rec = await delByIndex('recirculacion', 'byTechDate', [tech, date]);
  const med = await delByIndex('medidas', 'byTechDate', [tech, date]);
  const pur = await delByIndex('purga', 'byTechDate', [tech, date]);
  return { recirculacion: rec, medidas: med, purga: pur };
}


// --- Incidencias ---
export async function dbAddIncidencia(entry) {
  const db = await openDB();
  return new Promise((res, rej) => {
    const req = tx(db, "incidencias", "readwrite").add(entry);
    req.onsuccess = () => res(req.result);
    req.onerror = () => rej(req.error);
  });
}
export async function dbGetIncidencias(tech, date) {
  const db = await openDB();
  return new Promise((res, rej) => {
    const req = tx(db, "incidencias").index("byTechDate").getAll([tech, date]);
    req.onsuccess = () => res((req.result || []).sort((a, b) => (b.ts || 0) - (a.ts || 0)));
    req.onerror = () => rej(req.error);
  });
}
export async function dbGetIncidenciasAll(tech) {
  const db = await openDB();
  return new Promise((res, rej) => {
    const req = tx(db, "incidencias").index("byTechDate").getAll(IDBKeyRange.bound([tech, ""], [tech, "\uffff"]));
    req.onsuccess = () => res((req.result || []).sort((a, b) => (b.ts || 0) - (a.ts || 0)));
    req.onerror = () => rej(req.error);
  });
}
...
// --- Reset TOTAL (borra todo el trabajo, conserva ajustes en localStorage) ---
export async function dbClearAllData() {
  ...
  await clearStore("recirculacion");
  await clearStore("analisis");
  await clearStore("medidas");
  await clearStore("purga");
  await clearStore("incidencias");
  return true;
}
